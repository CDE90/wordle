...
def get_valid_input() -> str:
    while True:
        ...
        if len(guess) == 5 and guess.isalpha() and guess.lower() in words:
            ...
            output(
                Colour.RED, "Invalid input. Please enter a 5 letter word.", newline=True
            )
...
def game():
    ...
    while attempt <= 6:
        ...
        if guess == word:
            output(
                Colour.GREEN,
                f"You win! The word was {word}. It took you {attempt} attempts.",
                newline=True,
            )
            break
        else:
            output(
                Colour.RED,
                f"Incorrect. You have {6 - attempt} attempts left.",
                newline=True,
            )
            attempt += 1
    else:
